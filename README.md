...
Features: eight evenly spaced JPEG previews, browser-reported duration/dimensions, JSON observation export, browser print-to-PDF, and optional local WebGL night-vision and false-color preview styles. The GLSL styles remap visible RGB brightness; they do not recover infrared radiation or temperature. Styled previews are excluded from the observation report. It does not search for sources, detect AI generation, identify people or verify authenticity. Requested seek positions are not guaranteed exact decoded frame timestamps.
...
There is no daily/monthly quota: computation stays on the user's device and there is no shared compute service to exhaust. Client-side quotas would be bypassable and would offer no server-cost protection. The CSP blocks fetch/XHR and other network resources. CesiumJS, map tiles, terrain archives, OCR models and weather archives are not bundled here because this single-file tool keeps its offline, zero-request promise. The hosted ClipTrace service has separate authenticated spatial analysis.

## Verify a change

Use a synthetic four-second 320 × 180 MP4. Check eight visible previews and both visual styles against the original; download and inspect the JSON to confirm styled pixels were not substituted; download a JPEG; print to PDF; test cancellation; test an empty/malformed file; verify at 390px and desktop widths; confirm no network requests or JavaScript errors. Do not contribute private media. Browser and GPU support for WebGL varies; a filter failure should leave the original preview usable.
